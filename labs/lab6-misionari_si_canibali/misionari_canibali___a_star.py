...
class Graf:
    ...
    def fct_euristica_h2(self, nod_info):
        if (nod_info[1] + nod_info[2]) != 0 and nod_info[0] == 'est':
            return 2 * ((nod_info[1] + nod_info[2] - 2) // Nod.NR_LOCURI_BARCA) + 1
        elif (nod_info[1] + nod_info[2]) != 0 and nod_info[0] == 'vest':
            return 2 * ((nod_info[1] + nod_info[2] - 1) // Nod.NR_LOCURI_BARCA) + 2
        elif (nod_info[1] + nod_info[2]) == 0:
            return 0

    def muta_barca(self, nod_infoc, nr_mis, nr_can):
        # in laboratoarele anterioare, am avut aici "interschimba"
        nod_info = nod_infoc.copy()
        if nod_info[0] == 'est':
            nod_info[0] = 'vest'
            nod_info[1] -= nr_mis
            nod_info[2] -= nr_can
    ...
    def calculeaza_succesori(self, nod):
        ...
        if nod.info[0] == 'est':
            nr_mis_exist = nod.info[1]
            nr_can_exist = nod.info[2]
        else:
            nr_mis_exist = nod.info[3]
            nr_can_exist = nod.info[4]

        for mis in range(min(Nod.NR_LOCURI_BARCA, nr_mis_exist)+1):
            for can in range(min(Nod.NR_LOCURI_BARCA, nr_can_exist)+1):
                if mis >= can and mis + can >= 1 and mis + can <= Nod.NR_LOCURI_BARCA:
                    fiu = self.muta_barca(nod.info, mis, can)
                    if (fiu[1] >= fiu[2] and fiu[3] >= fiu[4]) or fiu[1] == 0 or fiu[3] == 0:
                        h = self.calculeaza_h(fiu)
                        l_succesori.append((Nod(fiu, h), 1))
