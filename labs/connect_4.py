...
class Joc:
    ...
    def final(self):
        ...
        for lin in self.matr: # lin = linie intreaga
            for col_idx in range(self.NR_COLOANE - 3):
                element = lin[col_idx]
                if element != '#':
                    OK = True
                    for col_idx_verif in range(col_idx + 1, col_idx + 4):
                        if element != lin[col_idx_verif]:
                            OK = False
        ...
        for col_idx in range(self.NR_COLOANE):  # lin = linie intreaga
            for lin_idx in range(self.NR_LINII - 3):
                element = self.matr[lin_idx][col_idx]
                if element != '#':
                    OK = True
                    for lin_idx_verif in range(lin_idx + 1, lin_idx + 4):
                        if element != self.matr[lin_idx_verif][col_idx]:
                            OK = False
        ...
        # diagonale \
        for lin_idx in range(self.NR_LINII):
            for col_idx in range(self.NR_COLOANE):
                diagonala = self.get_diagonala(lin_idx, col_idx)
                if len(set(diagonala)) == 1 and diagonala[0] != '#': # sau ...and '#' not in diagonala
                    return diagonala[0]

        # diagonale /
        for lin_idx in range(self.NR_LINII):
            for col_idx in range(self.NR_COLOANE):
                diagonala2 = self.get_diagonala2(lin_idx, col_idx)
                if len(set(diagonala2)) == 1 and diagonala2[0] != '#':  # sau ...and '#' not in diagonala
                    return diagonala2[0]
    ...
    def nr_intervale_deschise(self, jucator):
        ...
        for lin in self.matr:  # lin = linie intreaga
            for col_idx in range(self.NR_COLOANE - 3):
                lista_elem = []
                element = lin[col_idx]
                for col_idx_verif in range(col_idx, col_idx + 4):
                    lista_elem.append(lin[col_idx_verif])
                if juc_opus not in lista_elem:
                    rez += 1
        ...
        for col_idx in range(self.NR_COLOANE):  # lin = linie intreaga
            for lin_idx in range(self.NR_LINII - 3):
                ...
                element = self.matr[lin_idx][col_idx]
                for lin_idx_verif in range(lin_idx + 1, lin_idx + 4):
                    lista_elem.append(self.matr[lin_idx_verif][col_idx])
                if juc_opus not in lista_elem:
                    rez += 1
