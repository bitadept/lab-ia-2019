...
def alpha_beta(alpha, beta, stare):
    ...
    if stare.j_curent == Joc.JMAX:
        ...
        for mutare in stare.mutari_posibile:
            mutare_noua = alpha_beta(alpha, beta, mutare)
            if scor_curent < mutare_noua.scor:
                scor_curent = mutare_noua.scor
                stare.stare_aleasa = mutare_noua
                if mutare_noua.scor > alpha:
                    alpha = mutare_noua.scor
            if alpha >= beta:
                break
        ...
        for mutare in stare.mutari_posibile:
            ...
            if scor_curent > mutare_noua.scor:
                scor_curent = mutare_noua.scor
                stare.stare_aleasa = mutare_noua
                if mutare_noua.scor < beta:
                    beta = mutare_noua.scor
            if alpha >= beta:
                break
...
def main():
    ...
    while True:
        algoritm = int(input('ageleti 1 pentru minimax si 2 pentru alpha-beta: '))
        if algoritm in [1, 2]:
            break
